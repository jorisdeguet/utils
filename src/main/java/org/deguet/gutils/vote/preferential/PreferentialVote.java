...
public final class PreferentialVote {
	...
	public Integer safeRankFor(String c){
		Integer rank = rankFor(c);
		if (rank != null) return rank;
		// compute max rank
		int max = 0;
		for (String cand : this.candidates()){
			if (rankFor(cand)>max) max = rankFor(cand);
		}
		return max+1;
	}

	public boolean isBest(String candidate) {
		Integer rank = safeRankFor(candidate);
		for (String other : candidates()){
			if (safeRankFor(other) < rank) return false;
		}
		return true;
	}

	public boolean isWorst(String candidate) {
		int rank = safeRankFor(candidate);
		for (String other : candidates()){
			if (safeRankFor(other) > rank) return false;
		}
		return true;
