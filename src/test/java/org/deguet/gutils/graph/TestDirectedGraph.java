...
public class TestDirectedGraph{	
	...
	public void testCyclic(){
		DGraph<Integer,Integer> cycle  = buildCycle(new DGraphMatrix<Integer,Integer>(), 10).get2();
		DGraph<Integer,Integer> line  = buildLine(new DGraphMatrix<Integer,Integer>(), 10).get2();
	
		Assert.assertTrue(DGraphs.isCyclic(cycle));
		Assert.assertTrue(!DGraphs.isCyclic(line));
	}
	
	@Test
	public void testRestrictToUseless(){
		DGraph<Integer,Integer> cycle  = buildCycle(new DGraphMatrix<Integer,Integer>(), 10).get2();
		DGraph<Integer,Integer> restr  = DGraphs.restrictTo(cycle, cycle.vertices());
		assertEquals("Resteindre ne fait rien si on ne restreint pas ",cycle, restr);
	}
	
	@Test
	public void testRestrictTo(){
		DGraph<Integer,Integer> cycle  = buildCycle(new DGraphMatrix<Integer,Integer>(), 10).get2();
		Set<Integer> limited = cycle.vertices();
		limited.remove(limited.iterator().next());
		DGraph<Integer,Integer> restr  = DGraphs.restrictTo(cycle, limited);
		assertEquals("Resteindre ne fait rien si on ne restreint pas ",cycle.vertices().size()-1, restr.vertices().size());
	}
	...
	public static void main (String[] args){
		...
		myAssertTrue("Sinks equality  ", nullOrEquals(tbis.sinks(),mbis.sinks()));
	}
	...
	public static void test2(String prefix, DGraph m, DGraph r){
		...
		assertEquals("Get edge is the same ", r.getEdge(11, 9), m.getEdge(11, 9));
